...
`ffmpeg.exe` and `ffprobe.exe` are separate runtime programs. They are not copied into the MediaGrabber source tree and are not linked into the browser extension. The release workflow republishes the exact binaries as release assets; the installer downloads them and verifies pinned SHA-256 hashes.
...
`yt-dlp.exe` is a separate runtime program. The release installer downloads it from the MediaGrabber GitHub Release and verifies its pinned SHA-256 hash.

## Node.js runtime

The standalone CoApp and setup executable are Node.js Single Executable Applications (SEA). Release CI copies the Node 22.x executable and injects the MediaGrabber application blob.
...
TypeScript, esbuild, Vitest, happy-dom, and postject are development/build tools declared in npm metadata. They are not installed as separate end-user runtime programs by the MediaGrabber installer. Their exact versions and transitive dependency licenses are recorded in `package-lock.json`.
